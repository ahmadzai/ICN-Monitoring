...
start_date <- "2018-11-01"  # Start date, from where report will include submissions

end_date <- "2018-11-30"    # End date, till which report will have submissions ""
...
subchunkify <- function(g, fig_height=4, fig_width=9, print=FALSE) {
  ...
  sub_chunk <- paste0("
  `","``{r sub_chunk_", floor(runif(1) * 20000), ", fig.height=", fig_height, ", fig.width=", fig_width, ", echo=FALSE}",
  "\n(", 
    g_deparsed
    , ")()",
  "\n`","``
...
for (i in seq_along(province_list)) { 
 ...
 plot <- vecheatmap(df = districts_ind_long, 
                    ...
                    filtervar = "Provinces")
 subchunkify(plot, fig_height = heatmap_height_dist[i])
...
for (i in seq_along(districts_list)) { 
  ...
  subchunkify(plot, fig_height = heatmap_height_clstr[i])
...
```{r get_map, message=FALSE, warning=FALSE, cache = TRUE, eval=printmap, results='asis'}
...
if(kpi_df$monitor_fieldbookaccess%>%table()%>%length() > 0) {
    ...
    subchunkify(chart, fig_height = 2.5)
}
...
df = kpi_df
    ...
    chart <- evalbarplot(df, 
                ...
                title_break = 80)
    subchunkify(chart, fig_height = 3.2, fig_width = 8)
...
for (i in seq_along(province_list)) { 
  ...
  if(nrow(tmp) > 0) {
    ...
    subchunkify(plot, fig_height = height)
  }
